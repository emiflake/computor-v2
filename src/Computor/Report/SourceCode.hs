...
module Computor.Report.SourceCode where
...
prettySpanSquiggly lineOffset (Tag.Span
                                (SourcePos file (unPos -> startLine) (unPos -> startColumn))
                                (SourcePos _ (unPos -> endLine) (unPos -> endColumn)))
  sourceCode =
  let
    width =
      length (show (lineOffset + endLine)) + 2

    ln line =
      addLineNumber width (line + lineOffset) (pretty (Computor.Report.SourceCode.line line sourceCode))
  in
    case startLine of
      1 ->
        vsep $
          [ paddingBreak width
          , ln startLine
          , squigglyElongated width startColumn endColumn (Text.length (Computor.Report.SourceCode.line startLine sourceCode) + 1) (startLine /= endLine)
          , paddingBreak width
          ]
      _ ->
        vsep $
...
prettySpan :: Int -> Tag.Span -> Text -> Doc AnsiStyle
prettySpan lineOffset (Tag.Span (SourcePos file (unPos -> startLine) (unPos -> startColumn))
                           (SourcePos _ (unPos -> endLine) (unPos -> endColumn)))
  sourceCode =
  let
    width =
